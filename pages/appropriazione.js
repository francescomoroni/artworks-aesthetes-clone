...
export default function Lezardrieux() {
  ...
  const [showButtonMarketplace, setShowButtonMarketplace] = useState(false);
   ...
   const linkMarketplace = require("../components/link_appropriazione.js");
  ...
  function handleGoSearch() {
    if (insertedIndexToCheck > 0 && insertedIndexToCheck < 1000) {
      setIsNumberValid(true);
  ...
  useEffect(() => {
    ...
    splitImageInNParts("/appropriazione.png");
  }, []);
    ...
    <div className="flex flex-col justify-between min-h-screen">
      ...
      <div className="mt-4 bg-white ">
        ...
        <div className="text-center sm:py-4 sm:space-y-4">
          <p className="text-xl font-semibold sm:text-5xl font-Caslon">
            Enzo Fiore
          </p>
          <p className="text-lg sm:text-4xl font-Caslon">Appropriazione La Dama con l'Ermellino</p>
        </div>

        <div className="items-center justify-around pt-2 mx-4 sm:flex sm:justify-center sm:pt-6">
          {/* Quadro intero */}
          <img
            className={` w-96 border-0 border-black ${
              loadedCanvas ? "hidden" : "block "
            } `}
            src="/appropriazione.png"
            alt="Appropriazione La Dama con l'Ermellino
            ...
            {selectedIndex && (
              <>
                <div className="z-10 px-8 py-6 space-y-2 text-xl bg-white border-2 border-black rounded-lg">
                  ...
                  <p className="font-bold">{selectedIndex} / 999</p>
              ...
              <tr key={i}>
                {row.map((chunck, j) => (
                  <td
                    key={j + 1 + i * 27}
                    id={j + 1 + i * 27}
                    onClick={() => handleSelectedIndex(j + 1 + i * 27)}
                    className={`sm:p-0.5 hover:scale-[1.8] hover:cursor-pointer ${
                      insertedIndex === j + 1 + i * 27 ? "scale-[2] " : ""
                    } `}
                  >
                    <img key={j} src={chunck} width={20} alt="piece" />
                  </td>
                ))}
              </tr>
            ))}
